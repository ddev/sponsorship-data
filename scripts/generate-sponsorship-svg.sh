...
# Logo: viewBox 261.16x199.3, scaled to 20px tall → scale=0.10, width≈26px
# Positioned at (PAD, 5) so it centers on the 20px title text row (baseline y=22)
LOGO_SCALE=0.10
LOGO_W=26  # 261.16 * 0.10 ≈ 26
...
<svg xmlns="http://www.w3.org/2000/svg" width="100%" viewBox="0 0 ${CARD_W} ${CARD_H}" role="img" aria-label="DDEV Sponsorship: \$${total_formatted} / month">
  ...
  <g transform="translate(${PAD}, 5) scale(${LOGO_SCALE})" fill="${text_secondary}" fill-rule="evenodd">
  ...
  <text x="${TITLE_X}" y="22" font-family="system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif" font-size="20" font-weight="600" fill="${text_secondary}">DDEV Sponsorship</text>
  <text x="${PAD}" y="58" font-family="system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif" font-size="30" font-weight="700" fill="${text_primary}">${progress_display}% towards \$${target_formatted}/month goal</text>
  <rect x="${PAD}" y="72" width="${INNER_W}" height="${BAR_H}" rx="4" fill="${bar_track}"/>
  <rect x="${PAD}" y="72" width="${bar_fill_w}" height="${BAR_H}" rx="4" fill="${bar_fill_color}"/>
  <text x="${PAD}" y="104" font-family="system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif" font-size="20" fill="${text_secondary}">\$${total_formatted} / month</text>
</svg>
EOF
